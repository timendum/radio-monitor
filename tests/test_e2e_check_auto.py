...
import vcr
from test_e2e_check_by_user import mock_input_print
...
class E2ETestCaseCheckAuto(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            Path("test_e2e_check_auto.sqlite3").unlink()
        except FileNotFoundError:
            pass

        def test_conn_db(path=""):
            return cls.orig_db("test_e2e_check_auto.sqlite3")
    ...
    def test_1_virgin(self):
        ...
        with mock.patch("monitor.radio.virgin.parse", create=True) as mock_input:
            mock_input.side_effect = [("Cesare Cremonini", "50 Special - Record", "{mock:true}")]
        ...
        with utils.conn_db() as conn:
            ...
            self.assertEqual(title, "50 Special - Record")
            self.assertEqual(performer, "Cesare Cremonini")
    ...
    def test_2_match(self):
        ...
        with utils.conn_db() as conn:
            ...
            with my_vcr.use_cassette(
                "fixtures/e2e_virgin_spotify.yml", filter_headers=["Authorization"]
            ):  # type: ignore
                smatcher.main()
