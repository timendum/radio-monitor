...
def empty_songs_checks(self: unittest.TestCase, conn: Database):
    """Sanity check - empty tables"""
    rows = conn.fetch_many(..., "SELECT artist_id FROM artist")
    self.assertFalse(rows, "Table artist should be empty")
    rows = conn.fetch_many(..., "SELECT song_id, song_title FROM song")
    self.assertEqual(len(rows), 1, "Table song should be empty")
    self.assertEqual(rows[0][1], "TODO", "Table song contains only TODO")
    rows = conn.fetch_many(..., "SELECT song_id FROM song_artist")
    self.assertFalse(rows, "Table song_artist should be empty")
    rows = conn.fetch_many(..., "SELECT song_id, title FROM song_alias")
    ...
    rows = conn.fetch_many(..., "SELECT song_id FROM match_candidate")
    self.assertFalse(rows, "Table match_candidate should be empty")
    rows = conn.fetch_many(..., "SELECT song_id FROM play_resolution")
    self.assertFalse(rows, "Table play_resolution should be empty")


def one_play_checks(self: unittest.TestCase, conn: Database) -> tuple[str, str, str, str, int]:
    ...
    return station_name, title, performer, acquisition_id, play_id


def basic_match_checks(self: unittest.TestCase, conn: Database) -> str:
    # Play id
    p_rows = conn.fetch_many(..., "SELECT play_id, title_raw, performer_raw FROM play")
    ...
    # artist
    rows = conn.fetch_many(..., "SELECT artist_id, artist_name FROM artist")
    artist_ids = []
    for row in rows:
    ...
    rows = conn.fetch_many(..., "SELECT song_id, song_title, song_key FROM song")
    song_ids = []
    for row in rows:
        if row[1] == "TODO":
            continue
    ...
    for artist_id in artist_ids:
        rows = conn.fetch_many(
            ...,
            "SELECT artist_id FROM song_artist WHERE artist_id = ?",
            artist_id,
        )
        self.assertGreaterEqual(
            len(rows),
    ...
    for song_id in song_ids:
        rows = conn.fetch_many(..., "SELECT artist_id FROM song_artist WHERE song_id = ?", song_id)
        self.assertGreaterEqual(
            len(rows),
            1,
    ...
    rows = conn.fetch_many(
        int,
        "SELECT song_id FROM match_candidate WHERE play_id=?",
        play_id,
    )
    self.assertGreaterEqual(
    ...
    for song_id in rows:
        ...
        mc_rows = conn.fetch_many(
            tuple[int, str],
            "SELECT song_id, song_title FROM song WHERE song_id = ?",
            song_id,
        )
        self.assertEqual(
            len(mc_rows),
            1,
            f"Every mc should have one song row: {song_id} has {len(mc_rows)}",
        )
    # play_resolution by play_id
    rows = conn.fetch_many(
        tuple[int, str],
        "SELECT song_id, status FROM play_resolution WHERE play_id=?",
        play_id,
    )
    self.assertEqual(
        len(rows),
